...
async def _show_resolution_menu(client, message):
    ...
    text = "<b>🗜️ Select Compression Resolution</b>\n\n" \
...
async def _start_compression(client, message, callback_query, resolution):
    ...
    await callback_query.answer(f"Compression to {resolution} started!", show_alert=True)
                        ...
                        if any("Time Left" in line for line in text_lines):
                            ...
                            for i, line in enumerate(text_lines):
                                if "Time Left" in line:
                                    text_lines[i] = f"⏱️ <b>Time Left:</b> <code>{round(remaining, 1)} sec</code>"
                                    break
                        else:
