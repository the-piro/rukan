...
async def mega_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Error queuing download: {str(e)}")
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if success:
            await update.message.reply_text(
                f"✅ Download `{gid}` has been cancelled.", parse_mode="Markdown"
            )
        ...
        await update.message.reply_text(f"❌ Error cancelling download: {str(e)}")
